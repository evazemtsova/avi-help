...
**Демо:** `https://avi-help.vercel.app/` (фронт без изменений) + `https://avi-help-production.up.railway.app/` (бэк с hybrid).

### Задачи

- [x] Блок 0 — Подготовка журнала и baseline-снимок
- [x] Блок 1 — BM25 индекс на чанках
- [x] Блок 2 — RRF (Reciprocal Rank Fusion) и интеграция в retrieval pipeline
- [x] Блок 3 — Eval ablation (bi_only vs hybrid; bm25_only пропущен ради экономии бюджета)
- [x] Блок 4 — Анализ и решение по deploy (A: deploy hybrid as-is)
- [x] Блок 5 — Деплой на прод + замер latency
- [x] Блок 6 — Финальный прогон + декомпозиция + апдейт `eval_results.md`
...
- Получил: 4288 чанков, vocab 18433, build time **414–500ms** (порог <2s ✓). Sanity g020 находит статью 2831 на rank #1 (`2831_007`, score 23.097); pickle round-trip OK; lifespan-event на проде тестировался через `from main import _startup; _startup()` → `bm25_init_error: None`. Production smoke: push коммита `78bc36d` → Railway redeploy Success → `/health` стабильно возвращает `{"bm25_ready":true,"retrieval_ready":true}` на 5 sequential calls.
...
**Блок 2 — RRF + интеграция в retrieval pipeline:**
- Сделал: env-переменные `USE_HYBRID_RETRIEVAL` (default true), `HYBRID_CANDIDATES=20`, `RRF_K=60`; в `SearchHit` поля `bi_score: float = 0.0` (cosine) и `rrf_score: Optional[float] = None`; `_rrf_merge()` стандартная формула `Σ 1/(k+rank)` с pre-fetch BM25-only чанков из Chroma; `search()` теперь три взаимно-исключающих пути (reranker / hybrid / bi-only); fallback в `generate()` через `_query_bi_top1(hits) = max(h.bi_score for h in hits)` вместо `hits[0].score` (защита от RRF top-1 = BM25-only с bi_score=0); `last_search_timings` дополнен `bm25_ms` / `merge_ms` / `mode`.
- Получил: sanity 5 cherry-picked в hybrid → 4/5 в top-5 (g002 content gap, остальные ✓ как probe); 5 healthy → 4/5 без изменений + 1 микро-регрессия (g021 «звонят якобы из СБ» — BM25 boost'нул соседние safety-чанки `4221_*`, expected `1881_003` ушёл с #4 на >5); все 10 запросов дают bi_top1 ≥ 0.43 → fallback threshold 0.3 не false-positive; latency локально hybrid = 242ms (embed 214 + chroma 10 + bm25 15 + merge 3) — BM25 contribution ~20ms.
- Важно: `score` в hybrid режиме = rrf_score (~0.02-0.04 шкала, не cosine 0.5-0.7) — eval/log/API всё ещё работают потому что чтение `score` идёт через chunk_id, не численно. Решение `max(bi_score)` вместо `hits[0].bi_score` — устойчивее к edge case BM25-only-в-RRF-#1 (на 5 cherry такого не случилось, но теоретически возможно). Микро-регрессия g021 — известный риск RRF на лексически близких категориях; net effect посчитаем в Блоке 3 на 100 кейсах.

**Блок 3 — Eval ablation (bi_only vs hybrid):**
- Сделал: правки `eval.py` (configs `bi_only`/`mvp`/`baseline` развёл; `--retrieval-only` flag; BM25 init в main); прогон `bi_only` (cache-rebuild $2.77 одноразовый из-за коммита `5baa5e2` который сменил ANSWER_TOOL после Sprint 5 final eval); прогон `mvp` (hybrid) gen+judge $2.73; декомпозиция R@5/Faithfulness через failure case analysis.
- Получил: **Все 5 PRD-целей закрыты в hybrid**. Recall@5 0.8125→**0.8542** (+4.17 п.п., впервые ≥0.85), MRR@10 +0.17, Faithfulness full 0.66→**0.74** (+8 п.п., выше PRD revised 0.7), Relevance non-fb +0.09, Refusal **1.0** сохранено. Декомпозиция: 7 R@5 fixed (4 через BM25 only — g011/g020/g054/g073, expected не было в bi top-10; 3 RRF synergy — g025/g035/g060) − 3 broken (g021 RRF noise, g049/g051 BM25 boost'нул нерелевантные). Faithfulness: +21 fixed −13 broken = +8 net (Haiku даёт меньше overgeneralization на новых top-5).
- Важно: Sprint 6 Faithfulness +8 п.п. — bonus неожиданный (прогноз был −1..+2 п.п.); g002 «трек номер» теперь не fallback (потенциальная hallucination — content gap, Haiku пытается ответить); g050/g061 (демо-blocker'ы Sprint 5) теперь LLM-fallback в hybrid — roadmap query-нормализация «обяв→объявления». Bullet-fix урок: любая правка ANSWER_TOOL/SYSTEM_PROMPT инвалидирует весь LLM-кэш через cache-key payload — bi_only paid $2.77 это было.

**Блок 4 — Анализ + решение по deploy:**
- Сделал: декомпозиция Δ R@5 в 4 buckets (BM25 only / RRF synergy / content gap / другое); классификация всех 18 R@5=0 failure cases Sprint 6 bi_only baseline; sanity 5 контрольных кейсов R=1→R=1; deep-dive ответ на 3 вопроса пользователя (bullet-fix эффект, g023/g050/g061 в hybrid, faithfulness +8 декомпозиция).
- Получил: **Решение A — deploy hybrid as-is**. 18 R@5=0 → 7 fixed (4 BM25 only + 3 RRF synergy) + 6 content gap (структурные дыры БЗ) + 5 «not in top-5 но в top-20» (RRF не вытянул) − 3 broken (g021/g049/g051) = net +4 кейса. Faithfulness honest = **+5..+7 п.п. устойчивых** (часть 8 п.п. — Sonnet noise; декомпозиция a=30-40% BM25 cluster effect, b=0% (g050/g061 ушли в LLM-fallback наоборот), c=40-50% style shift, noise=10-20%). g061 demo-blocker НЕ снят — query-нормализация «обяв→объявления» в Sprint 7 roadmap.
- Важно: **Methodological finding #6 (новая для проекта)** — bullet-fix эффект: косметическая правка `ANSWER_TOOL` сменила format ответов Haiku (30/97 → 0/97 ответов с `•`, 18/97 → 83/97 с `- ` lists) и стоила $2.77 на cache rebuild. Faithfulness потерял −4 п.п. на одной retrieval из-за: 70% format-induced specificity (Sonnet парсит `- ` items как discrete claims), 30% реальные content-shifts (g018). Урок: смена tool/system промптов инвалидирует cache как retrieval-правка; формат вывода влияет на judge так же сильно как содержание.

**Блок 5 — Деплой на прод + 30 sequential latency:**
- Сделал: 2 деплоя (`ec03067` main + `eed6ae3` hotfix latency-полей — `bm25_ms`/`merge_ms` существовали в `last_search_timings` но не пропагировались в API response/logs); smoke 5 запросов через `/answer/sync` (g020 вернул статью 2831 — cherry-pick win на проде; g061 → LLM-fallback; ood19/ood20 → competitor pre-LLM); 30 sequential latency-replay с теми же IDs что Sprint 5 v3 → `data/eval/prod_latency_v4/results.json`.
- Получил: **Все stop-conditions пройдены.** Client P50 4798ms (vs Sprint 5 v3 4562ms, +236ms) ≤5s ✓; Client P95 7315ms (vs 7317ms, −2ms) ≤8s ✓; Server P95 −1068ms (5852ms vs 6920ms — variance уменьшилась); Server retrieval P50 151ms ≤500ms ✓✓ (3× запас). **BM25 P50=10ms, P95=25ms** на shared Railway CPU — в **1000× быстрее** чем reranker (Sprint 5 Блок 5 P50=8500ms). n_fallback in-domain 0/30 (Sprint 5 v3 имел 1 — g002, теперь Haiku пытается ответить на content gap → hallucination риск).
- Важно: BM25 contribution на Railway shared CPU укладывается в +25ms P95 (vs +200-300ms прогноза брифа) — IDF-lookup в Python dict работает консистентно даже без MKL/AVX. Это архитектурная валидация: **алгоритмические retrieval-фильтры (BM25/SPLADE) >> нейросетевые (cross-encoder) на shared-CPU**. g002 регрессия (fallback→hallucination) — потенциальный quality issue для content-gap, roadmap Sprint 7 HyDE / explicit gap-detector.

**Блок 6 — Финальный прогон + sprint6 summary + eval_results.md:**
- Сделал: финальный mvp rerun из cache (308/308 hits, $0 paid, 1.3s); финальная сводка в `sprint6_changes_log.md` (cumulative-таблица Sprint 4→5→6, декомпозиция R@5 4 buckets, faithfulness +5..+7 honest range, 6 методологических находок); архивировал `eval_results.md` → `eval_results_sprint5.md`, написал новый `eval_results.md` под Sprint 6 final state с конфигурацией hybrid retrieval по умолчанию.
- Получил: **8/9 PRD-целей закрыто** (cost — единственный недобор, требует prompt caching); финальный mvp run подтвердил Recall@5 0.8542, MRR@10 0.7024, Faith full 0.7400, Refusal 1.0; cumulative spend Sprint 6 = $5.70 / $21 budget (с +$6 пополнением), остаток $6.26 на Sprint 7.
- Важно: PRD revision proposal вынесен в чат (3 пункта: F11 latency → TTFB-декомп с конкретными цифрами Sp6 prod replay; 7.2 Faithfulness ≥0.7 окончательно (закрыта Sp6); 7.3 Cost ≤$0.005 → ослабить до ≤$0.007 ИЛИ принять prompt caching как roadmap-условие). Sprint 6 ✅ закрыт, переход в Sprint 7.
...
## Спринт 7 — Query-вариативность (опечатки, синонимы, сленг)

**Статус:** ⏳ in progress
**Цель:** закрыть Sp6 roadmap-кандидаты по query-вариативности — опечатки (g042/g058/g054), сленг/сокращения (g061 «обяв→объявления»), content-gap mitigation (HyDE/multi-query). Меряем дельту R@5/MRR + честный side-effect breakdown как в Sp6.
**Принцип:** одна правка = одна ablation = одна запись в `docs/sprint7_changes_log.md` (TBD после Блока 1).

### Задачи

- [x] Блок 1 — Spell-correction (SymSpell, vocab из BM25 corpus)
- [x] Блок 2 — Деплой на прод + smoke (Spell)
- [~] Блок (skip) — Query-нормализация / синонимы (g061 «обяв») — решено пропустить
- [ ] Блок (TBD) — HyDE / multi-query rewrite для content-gap кейсов

### Заметки

**Блок 1 — Spell-correction (SymSpell-like, vocab из BM25):**
- Сделал: SymSpell-like corrector (delete-1 index, edit-distance ≤ 1 с transposition) на vocab из BM25-корпуса; интеграция в `retrieval.search()` перед обоими ретриверами под env `USE_SPELL_CORRECTION=true` (default), коррекции пробрасываются в `last_search_timings` → JSONL логи; lifespan-init после BM25 + `spell_ready` в `/health`; eval.py подхватывает для ablation.
- Получил: vocab=18433, build 100ms, query latency <1ms. Retrieval-only mvp: **R@5 0.8542→0.8646 (+1 case), MRR@10 0.7024→0.7089**. g054 «верниите» rank 4→**1** (MRR boost), g042 «пороль» / g058 «вывыести» None→6 (движение, не закрыло top-5), g034 «слила доступ» 6→**5** (FIX, bonus не-typo), g016 «бабками» 1→2 (микро-регрессия). Cost $0.01.
- Важно: vocab узкий (домен Авито) → false positive «забыл»→«забыли» (БЗ пишет вежливо во мн.ч.) — для retrieval нейтрально, но виден в логах; tuning через `min_cand_freq` отложен до prod replay.
- Важно: 8 из 10 typo-кейсов золота уже вытаскивались BM25-лексикой по другим словам — spell-фикс закрывает пользовательский баг «как поменять пороль ≠ как поменять пароль» (top-1 sanity ✓), но дельта на eval мала из-за уже-низкого числа typo-only failure'ов; больший выигрыш ожидаем на prod-трафике с реальными опечатками вне golden set.

**Блок 2 — Деплой на прод + smoke:**
- Сделал: commit `2602058` → push origin/main → Railway auto-deploy; poll `/health` до `spell_ready=true`; smoke 5 запросов через `/search` (3 typo + 2 контрольных) на prod URL.
- Получил: `/health` = `{retrieval_ready, bm25_ready, spell_ready}` все true. «как поменять пороль» = «как поменять пароль» (одинаковый top-3, статья 4336) ✓; g054 «верниите деньги» top-1 = 2865 expected ✓; g042 «забыл пороль» top-3 = статья 4295 (релевантно); latency server-retrieval 147-231ms (без оверхеда от spell vs Sp6 baseline 151ms P50).
- Важно: smoke через `/search` не возвращает corrections в response — для аудита коррекций смотреть `data/logs/requests_*.jsonl` (поля `latency_ms.original_query`/`corrections`, добавлены в Блоке 1). Полноценный 30-sequential latency replay не делал — Spell <1ms на retrieval-only eval, server-side overhead в smoke 5 запросов не виден.
